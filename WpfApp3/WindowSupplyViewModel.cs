...
namespace WpfApp3
{
    class WindowSupplyViewModel : ViewModelBase
    {
        ...
        private void GenSupply()
        {
            ...
            dt.DefaultView.Sort = "ROOT_DEMAND,LLC,ROOT_MATERIAL,DEMAND,DEMAND_DATE,SPLIT_NO";
            int cnt = 0;
            string ppid = "";
            string pred = "";
            foreach (DataRow dr in dt.DefaultView.ToTable().Rows)
            {
                SUPPLY sp = new SUPPLY()
                {
                    ...
                    RoutingActionLabel = makeRoutingActionLabel(dr["ROUTING_ACTION"].ToString()),
                    SupplyID = (dr["DEMAND"].ToString().Equals(pred) ? dr["DEMAND"].ToString() + "_" + dr["LLC"].ToString() + "_" + dr["SPLIT_NO"].ToString() + "_0" : dr["DEMAND"].ToString() + "_" + dr["LLC"].ToString() + "_" + dr["SPLIT_NO"].ToString()),
                    SupplyPID = dr["PARENT_DEMAND"].ToString()+"_"+(Convert.ToInt32(dr["LLC"])-1).ToString()+"_0"
                   // SupplyID = dr["DEMAND"].ToString() + "_" + dr["PARENT_DEMAND"].ToString() + "_" + dr["LLC"].ToString() + "_" + dr["SPLIT_NO"].ToString(),
                    //SupplyPID = (Convert.ToInt32(dr["LLC"]) == 0 ? (Convert.ToInt32(dr["SPLIT_NO"]) == 0 ? "" : dr["DEMAND"].ToString() + "_" + dr["PARENT_DEMAND"].ToString() + "_" + dr["LLC"].ToString() + "0") : dr["PARENT_DEMAND"].ToString() + "_" + ppid + "_" + (Convert.ToInt32(dr["LLC"]) - 1).ToString() + "_0")

                };
                //sp.RowPNo = FindParentRowID(sp, supplyList);
                supplyList.Add(sp);
                ppid = sp.ParentDemand;
                pred = sp.Demand;
                cnt++;
            }
        ...
        private string makeRoutingActionLabel(string ra)
        {
            string tag;
            if (ra.Length > 14)
            {
                tag = ra.Substring(0, 15) + "..]";
            }
            else
            {
                tag = ra;
        ...
        public void ZoomFactor(IEnumerable<SUPPLY> supply)
        {
            int cnt = 0;
            using (IEnumerator<SUPPLY> enumerator = supply.GetEnumerator())
            {
